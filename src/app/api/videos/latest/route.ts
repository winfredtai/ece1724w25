...
type VideoTask = Database['public']['Tables']['video_generation_task_statuses']['Row'] & {
...
export async function GET() {
  try {
    ...
    const { data: videoTasks, error } = await supabase
      ...
      .limit(5) as { data: VideoTask[] | null; error: any };

    if (error) {
      console.error('获取视频失败:', error);
      return NextResponse.json({ error: error.message || '获取视频失败' }, { status: 500 });
    }
